...
(defrule price-risk-factor-1
    ...
    =>
    (assert(price-risk-factor(name ?name)(percentage 0))))
...
(defrule total-average-risk
    ...
    (bind ?total (/ ?sum 5))
    (bind ?premium (* 700 (+ 1 ?total)))
    (assert(insurance-premium(name ?name)(premium ?premium)))
	(printout t "The Insurance Premium of " ?name " is " ?premium crlf)) 
